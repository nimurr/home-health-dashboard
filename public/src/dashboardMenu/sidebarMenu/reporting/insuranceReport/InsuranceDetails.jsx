...
import { useGetByIdInsurenceQuery } from "../../../../redux/features/reporting/getByIdInsurence";
...
const InsuranceDetails = () => {
  ...
  const {data: singleInsurence, error} = useGetByIdInsurenceQuery({id, date})
  // console.log(singleInsurence);
  ...
  return (
    ...
    <div className="flex items-center gap-2">
          <p className="cursor-pointer" onClick={() => navigate('/dashboard/reporting/insurancereport')}> 
        <BiArrowBack />
          </p>
          <h1 className="text-xl font-semibold">Insurence-Reporting Details</h1>
        </div>
      ...
      error ? (
        <div className="text-red-500 text-center mt-10">{error?.data?.message}</div>
         ...
         <div>
           ...
           {singleInsurence?.data?.attributes?.map((details, index) => {
