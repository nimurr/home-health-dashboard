...
import { useAllIncidentQuery } from "../../../../redux/features/reporting/getAllincident";
...
const IncidentReport = () => {
   ...
   const [name, setName] = useState(' ')
    ...
    const {data: allIncident,error, isLoading} = useAllIncidentQuery(name)
    // console.log(allIncident);
    
 
    const onActionClick = (record) => { 
    
        navigate(`detail/${record?.id}`, { state: { details: record } });
    ...
    <div className="">
      ...
      <div className="rounded-t-lg mt-[24px]">
        <div className="flex py-[22px] justify-between items-center">
          ...
          <p className=" text-[24px]">Incident-Reporting Form</p>
          </div>
           
          <div>
          <button
              onClick={() => navigate("/dashboard/reporting/seeincident")}
...

          {
            error ? (
              ...
              <div>
                ...
                <Table
                 pagination={{
                  total: allIncident?.data?.attributes.length,
                  ...
                  columns={columns}
                  dataSource={allIncident?.data?.attributes}
